...
var StateController = function ( dispParams ) {
	...
	function computeMovement( x, y, previousPosition ) {
		/* (2.1.1.1) Mouse Movement */
		const cursorDisplacement = new THREE.Vector2((x-previousPosition.x),(y-previousPosition.y));
	...
	function updateProjectionParams( e, movement ) {
		...
		/* (2.3.1) Implement Perspective Transform */

		var nextClipNear = _this.state.clipNear - movement.y; // If movement.y positive will subtract bringing near plane closer, if movement.y is negative will add bring clip plane further away
		if (nextClipNear >= 1){
			_this.state.clipNear = nextClipNear;
		}
	}
